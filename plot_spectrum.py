...
fig, axes = plt.subplots(1,3, figsize=(12,3), width_ratios=(2,5,5), constrained_layout=True)
...
#sec_ses = sec0.reduce_time(reduce_by=(lambda x:tt_ses(x, H=2400.0)), t_range=(-0.5, 0.5))
#sec_sbs = sec0.reduce_time(reduce_by=(lambda x:tt_sbs(x, H=2950.0)), t_range=(-0.5, 0.5))
...
ax.text(0.01, 0.99, '(b)', horizontalalignment='left',
                         verticalalignment='top',
                         transform=ax.transAxes,
                         backgroundcolor='white',
                         fontsize=12)
...
ax.text(0.01, 0.99, '(c)', horizontalalignment='left',
                         verticalalignment='top',
                         transform=ax.transAxes,
                         backgroundcolor='white',
                         fontsize=12)

fig.colorbar(p, ax=axes[1:3], orientation='vertical', fraction=0.1, ticks=[1e-12, 2e-12, 3e-12], location='right', label='Spectra amplitude')
plt.savefig("spec2.pdf", dpi=400, bbox_inches='tight')
